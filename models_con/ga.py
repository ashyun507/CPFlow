...
class GAEncoder(nn.Module):
    ...
    def forward(
        ...
        rigid_mask = (
            rigid_update_mask.bool() if rigid_update_mask is not None else node_mask
        )
        ...
        for b in range(self._ipa_conf.num_blocks):
            ...
            seq_tfmr_out = self.trunk[f'seq_tfmr_{b}'](
                node_embed, src_key_padding_mask=(~node_mask.bool()))
            node_embed = node_embed + self.trunk[f'post_tfmr_{b}'](seq_tfmr_out)
            node_embed = self.trunk[f'node_transition_{b}'](node_embed)
            node_embed = node_embed * node_mask[..., None]
            rigid_update = self.trunk[f'bb_update_{b}'](
                node_embed * rigid_mask[..., None])
            curr_rigids = curr_rigids.compose_q_update_vec(
                rigid_update, rigid_mask[..., None])
        ...
        if return_node_embed:
            return pred_rotmats1, pred_trans1, pred_angles1, pred_seqs1_prob, node_embed
